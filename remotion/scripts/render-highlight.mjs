...
const stems = {highlightVertical:"asmi-story-v4-vertical",highlightWidescreen:"asmi-story-v4-widescreen"};

for (const id of ids) {
  const composition=await selectComposition({serveUrl:bundled,id,puppeteerInstance:browser});
  if(process.env.STILLS === "1") {
    for(const frame of [38,105,180,235,300,355,420,490,558,585]) await renderStill({composition,serveUrl:bundled,output:`/tmp/${id}-v4-${frame}.png`,frame,puppeteerInstance:browser});
    continue;
  }
